...
nb-container
  ...
  view(:style="[{height: 45, backgroundColor: '#FFF'}, styl.borderB1,styl.rowStart]")
    ...
    nb-input(placeholder="请输入关键词查询" ,:style="[styl.ft14]", v-model="searchKey", :onSubmitEditing="textSearch", v-if="isIOS8")
    nb-input(placeholder="请输入关键词查询" ,:style="[styl.ft14]", :onChangeText="searchChange", :value="searchKey", v-else)
      ...
      view(:style="[styl.bgWhite, styl.pad10, styl.mt10]")
        touchable-opacity(:on-press="choose.bind(this, args.item)", :style="styl.col")
          ...
          view(:style="[styl.rowStart, styl.mt5]", v-if="args.item.compAddr")
            ...
            text(:style="[styl.pl5, styl.color88, styl.ft12]") {{args.item.compAddr}}
